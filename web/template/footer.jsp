<%--
  Created by IntelliJ IDEA.
  User: Ezreal
  Date: 2016/12/6
  Time: 11:42
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<!-- End Main -->
<!-- Start Links -->
<div id="links">
    <div class="cont">
        <div id="fastlinks">
            <h3>常用链接</h3>
            <ul
            ><li><a target="_blank" href="http://59.77.226.89/Jmail/">校领导信箱</a></li
            ><li><a target="_blank" href="./html/xndh">校内电话</a></li
            ><li><a target="_blank" href="http://xwgk.fzu.edu.cn/html/xwgz/zbcg/">招标采购</a></li
            ><li><a target="_blank" href="http://xxb.fzu.edu.cn">信息化建设办公室</a></li
            ><li><a target="_blank" href="http://fzdxb.cuepa.cn/">福州大学报</a></li
            ><li><a target="_blank" href="http://www.lib.fzu.edu.cn/">图书馆</a></li
            ><li><a target="_blank" href="http://rsc.fzu.edu.cn/zpindex.php">招聘信息</a></li
            ><li><a target="_blank" href="http://xyzh.fzu.edu.cn/">校友会基金会</a></li
            ><li><a target="_blank" href="http://met.fzu.edu.cn/">现代教育技术中心</a></li
            ></ul>
        </div>
        <ul id="topiclinks"
        ><li class="pic"><a href="http://news.fzu.edu.cn/html/ztxw/lxyz/1.html" target="_blank" ><img src="./attach/2016/05/06/220432.jpg"></a></li
        ><li class="pic"><a href="http://i.fzu.edu.cn" target="_blank" ><img src="./attach/download/2015/05/10/119512.jpg"></a></li
        ><li class="sub"><a href="http://weibo.com/fzuniversity" target="_blank"><img src="./themes/30480/fzu_new/images/weibo.png"></a><img src="./themes/30480/fzu_new/images/weixin.png"></li
        ><li class="sub"><a href="http://map.fzu.edu.cn/" target="_blank"><img src="./themes/30480/fzu_new/images/maps.png"></a><a href="http://m.fzu.edu.cn" target="_blank"><img src="./themes/30480/fzu_new/images/mobile.png"></a></li
        ></ul>
    </div>
</div>
<!-- End Links -->
<!-- Start Footer -->
<div id="footer">
    <table cellspacing="0" class="cont">
        <tr>
            <td width="82"><script type="text/javascript">document.write(unescape("%3Cspan id='_ideConac' %3E%3C/span%3E%3Cscript src='http://dcs.conac.cn/js/14/000/0000/41426288/CA140000000414262880003.js' type='text/javascript'%3E%3C/script%3E"));</script></td>
            <td>
                <span id="icp">闽ICP备05005463号</span>
                <span class="info"><span>&copy; 2015 福州大学</span></span>
                <span class="info"><span>地址：福建省福州市福州地区大学新区学园路2号</span><span>邮编：350116</span><span>传真：86-0591-22866099</span></span>
            </td>
        </tr>
    </table>
</div>
<script type="text/javascript">
    i=0;
    $(".list-tab tr").each(function(){
        if(Math.floor(i)%2==0){
            $(this).css("background","#F9F9F9");
        }
        i++;
    });
    j=0;
    $(".list-tab td").each(function(){
        if(Math.floor(j)%2==1){
            $(this).css("border-left","solid 1px #ddd");
        }
        j++;
    });
</script>
<!-- End Footer -->
</div>
<!-- End All -->
