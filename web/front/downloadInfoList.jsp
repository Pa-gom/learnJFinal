...
<div id="main">
    ...
    <div id="rside2">
        ...
        <ul class="list_main_content">
            <#list downloadList.getList() as x>
                <li>
                    <span>${x.empPublishDate}</span>
                    <h3><a href="/downloadDetail/${x.id}" title="${x.empTitle}">${x.empTitle}</a></h3>
                </li>
            </#list>
        </ul>
        <script>
            function changeUrl() {
                ...
                url = url.replace("/downloadList/1", "/downloadList/" + num);
        ...
        <div class="ecms_pag">
            <div class="ecms_pagination">
                <ul>
                    <#if currentPage gt 1>
                        <a class="ecms_disablepage" href="/downloadList/${currentPage-1}"><</a>
                    </#if>
                    <#list (currentPage-5)..(currentPage-1) as s>
                        <#if s gt 0>
                            <li><a class="ecms_pagenav" href="/downloadList/${s}">${s}</a></li>
                        </#if>
                    </#list>
                    ...
                    <#if currentPage lt pageCount>
                        <li class="nextpage"><a href="/downloadList/${currentPage+1}">></a></li>
                    </#if>
                </ul>
            </div>
            <div class="ecms_go">
                跳转：<input type="text" class="ecms_jump_input" id="pageNum" onchange="changeUrl()"/> 页 <a
                    style="cursor:hand" class="list_p6" id="goToUrl" href="/downloadList/1">GO</a>
            </div>
