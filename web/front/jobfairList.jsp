    ...
    <div id="rside2">
        ...
        <ul class="list_main_content">
            <#list infoList.getList() as x>
                <li>
                    <span>${x.empJobFairPublishDateTime}</span>
                    <h3><a href="/jobfairDetail/${x.empJobFairNo}" title="${x.empJobFairTitle}">${x.empJobFairTitle}</a></h3>
                </li>
            </#list>
        </ul>
        <script>
            function changeUrl() {
                ...
                url = url.replace("/jobfairList/1", "/jobfairList/" + num);
        ...
        <div class="ecms_pag">
            <div class="ecms_pagination">
                <ul>
                    <#if currentPage gt 1>
                        <a class="ecms_disablepage" href="/jobfairList/${currentPage-1}"><</a>
                    </#if>
                    <#list (currentPage-5)..(currentPage-1) as s>
                        <#if s gt 0>
                            <li><a class="ecms_pagenav" href="/jobfairList/${s}">${s}</a></li>
                        </#if>
                    </#list>
                    ...
                    <#if currentPage lt pageCount>
                        <li class="nextpage"><a href="/jobfairList/${currentPage+1}">></a></li>
                    </#if>
                </ul>
            </div>
            <div class="ecms_go">
                跳转：<input type="text" class="ecms_jump_input" id="pageNum" onchange="changeUrl()"/> 页 <a
                    style="cursor:hand" class="list_p6" id="goToUrl" href="/jobfairList/1">GO</a>
            </div>
